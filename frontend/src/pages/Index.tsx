...
const Index = () => {
  ...
  useEffect(() => {
    if (showContent && featuresSectionRef.current) {
      window.scrollTo({
        top: featuresSectionRef.current.offsetTop - 50, // 50px 위로 스크롤 (조정 가능)
        behavior: 'smooth',
      });
    }
  }, [showContent]);
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (!isTransitioning && window.scrollY < 200 && showContent) { // 스크롤 위치가 200px 미만이고 콘텐츠가 보이는 상태일 때
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      {/* Hero Section */}
       <div onClick={() => {
          ...
          }, 400); // Hero Section 전환 시간 (300ms)보다 길게 설정
        }} className={`text-center min-h-screen flex flex-col justify-center transition-all duration-300 ease-in-out ${showContent ? 'translate-y-[-100vh] opacity-0' : 'translate-y-0 opacity-100'}`} ref={heroSectionRef}> 
        <div className="max-w-3xl mx-auto">     
           <h1 className="text-5xl md:text-7xl font-bold text-foreground/90 mb-6">
            AI와 함께하는
          ...
          <div ref={featuresSectionRef} className="py-16">
            <div className="py-16">
              <div className="text-center mb-16">
                <h2 className="text-4xl font-extrabold text-foreground/80 mb-4">
              ...
              <div className="grid md:grid-cols-3 gap-12 max-w-5xl mx-auto">
                {[
                  ...
                  <div key={index} className="text-center p-6 bg-foreground/10 rounded-xl shadow-md transform transition duration-300 hover:scale-105">
                    ...
                    <h3 className="text-xl font-bold text-forground mb-3">
                      {process.title}
                    </h3>
                    <p className="text-forground/10 leading-relaxed">
                      {process.description}
                    </p>
                  </div>
                ))}
